...
export const following = async (currentUserId, _id) => {
    const { data } = await axios.post(`http://localhost:5000/api/user/suggested/${currentUserId}`, {
        idUserFlow: _id,
    });
    console.log(data);
};
export const unFollowing = async (currentUserId, _id) => {
    const { data } = await axios.post(`http://localhost:5000/api/user/unflow/${currentUserId}`, {
        idUserFlow: _id,
    });
    console.log(data);
};
export const getCurrentUserByID = async (id) => {
    const res = await axios.get(`http://localhost:5000/api/user/get-user-by-id/${id}`);
